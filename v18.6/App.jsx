...
function ScanScreen({gear,types,lists,usage,currentUser,onScanResult,onQuickSignIn}){
  ...
  const myOutCount=useMemo(()=>{const set=new Set(usage.filter(u=>u.instructor===currentUser.name&&u.gear_id&&u.signed_in_out==="OUT"&&!u.time_in).map(u=>u.gear_id));return [...set].filter(id=>{const g=gear.find(x=>x.gear_id===id);return g&&g.signed_in_out==="OUT";}).length;},[gear,usage,currentUser]);
